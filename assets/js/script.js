...
const searchInput = document.querySelector('#pesquisando')
...
function manipularDom() {
  jsonArray.forEach((e, index) => {
    ...
    <ul class="searchElements" onclick="mostraModal(${index})">
     ...
     <li class="cards flex"><h3>Types:</h3><span>${e.types
...
function search() {
  const searchH2 = document.querySelectorAll(".searchElements li h2")
  let searchElement = document.querySelectorAll(".searchElements")
  let valueSearch = searchInput.value.toLowerCase()


  searchH2.forEach((element, index) => {
    if (element.innerHTML.toLowerCase().includes(valueSearch)) {
      searchElement.forEach(() => {
        searchElement[index].style.display = ''
      })
    }else {
      searchElement[index].style.display = 'none'
    }
  })
